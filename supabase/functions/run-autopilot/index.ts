...
async function generatePersonalizedEmailWithIntelligence(openai: OpenAI, lead: { business_name: string; email: string; website?: string; rating?: number; review_count?: number; address?: string }, campaign: { niche: string; location: string }, intelligence: Awaited<ReturnType<typeof getLeadIntelligence>>, intentApproach: { approach: string; hooks: string[] }, userPrefs: { brand_voice?: string; creativity_level?: number; avoid_phrases?: string[] } | null): Promise<{ subject: string; body: string; tokens: Record<string, string> }> {
  ...
  const completion = await openai.chat.completions.create({ model: 'gpt-4o', messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }], temperature: userPrefs?.creativity_level || 0.75, max_tokens: 800 });
  const body = completion.choices[0]?.message?.content?.trim() || '';
  const subjectCompletion = await openai.chat.completions.create({ model: 'gpt-4o', messages: [{ role: 'system', content: 'Write a compelling 5-8 word email subject line. Be specific and relevant. Avoid spam triggers.' }, { role: 'user', content: `Subject line for email to ${capitalizedName} at ${businessName}:\n\n${body}` }], temperature: 0.8, max_tokens: 50 });
  const subject = subjectCompletion.choices[0]?.message?.content?.trim().replace(/^["']|["']$/g, '') || `Quick question about ${businessName}`;
