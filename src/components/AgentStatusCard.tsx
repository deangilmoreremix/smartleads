...
export default function AgentStatusCard({
  ...
  const getStatusBgColor = () => {
    if (isComplete) return 'bg-gradient-to-br from-amber-400 to-orange-500';
    if (isFailed) return 'bg-gradient-to-br from-red-500 to-red-600';
    return 'bg-gradient-to-br from-amber-400 to-orange-500 animate-pulse';
  };
    ...
    <div className="bg-white rounded-lg shadow-lg p-8 border border-amber-200">
      ...
      <div className="space-y-3">
        {steps.map((step, index) => (
          ...
          >
            ...
            {step.status === 'in_progress' && (
              <Loader2 className="w-5 h-5 text-orange-500 flex-shrink-0 animate-spin" />
            )}
            {step.status === 'failed' && (
      ...
      {isComplete && Object.keys(resultData).length > 0 && (
        <div className="mt-6 pt-6 border-t border-amber-200">
          <h3 className="text-sm font-semibold text-stone-700 mb-3">Results:</h3>
          <div className="grid grid-cols-2 gap-3">
            {resultData.leadsFound !== undefined && (
              <div className="bg-amber-50 rounded-lg p-3 text-center border border-amber-200">
                ...
                <div className="text-xs text-orange-700 mt-1">Leads Found</div>
              </div>
            )}
            {resultData.emailsGenerated !== undefined && (
              <div className="bg-amber-50 rounded-lg p-3 text-center border border-amber-200">
                ...
                <div className="text-xs text-orange-700 mt-1">Emails Generated</div>
              </div>
            )}
